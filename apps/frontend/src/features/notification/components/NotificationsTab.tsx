  ...
  return (
    <div className="space-y-3">
      {[0, 1, 2].map((index) => (
        <div
          key={index}
          className="flex items-center gap-3 rounded-lg border border-white/8 bg-sos-inset p-4"
        >
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-lg border border-white/10 px-3 py-2 text-xs font-semibold text-slate-300 transition hover:border-emerald-300 hover:text-white disabled:cursor-not-allowed disabled:border-white/5 disabled:text-slate-600"
        >
          모두 삭제
        </button>
      </div>
      ...
      {notifications.length === 0 ? (
        <div className="rounded-lg border border-white/8 bg-sos-inset p-6 text-center">
          <p className="text-sm font-medium text-slate-300">받은 알림이 없어요.</p>
          <p className="mt-1 text-sm text-slate-500">새 소식이 도착하면 여기에 표시됩니다.</p>
        </div>
      ) : (
        <ul className="space-y-3">
          {notifications.map((notification) => (
            <li
              key={notification.id}
              className="flex items-start gap-3 rounded-lg border border-white/8 bg-sos-inset p-4 transition hover:border-emerald-300/40"
            >
              <span
                aria-hidden="true"
              ...
              <div className="flex shrink-0 gap-2">
                {!notification.isRead ? (
                  <button
                    ...
                    className="rounded-md border border-white/10 px-2 py-1 text-xs font-medium text-slate-300 transition hover:border-emerald-300 hover:text-white"
                  >
                    읽음
                  </button>
                ) : null}
                <button
                  ...
                  className="rounded-md border border-white/10 px-2 py-1 text-xs font-medium text-slate-300 transition hover:border-red-300 hover:text-red-200"
                >
                  삭제
                </button>
              </div>
