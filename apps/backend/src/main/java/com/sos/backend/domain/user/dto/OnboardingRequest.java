...
public record OnboardingRequest(
    ...
    @ArraySchema(
        ...
        arraySchema = @Schema(description = "질문1: 소비 활동 (다중선택, NONE은 배타 선택)", example = "[\"ENTERTAINMENT\",\"TRAVEL\"]")
    )
    @NotEmpty(message = "economicActivities는 1개 이상 선택해야 합니다")
    @JsonAlias("economicActivities")
    @JsonProperty("economic_activities")
    ...
    @JsonAlias("communicateChannels")
    @JsonProperty("communicate_channels")
    ...
    @JsonAlias("onlineActivities")
    @JsonProperty("online_activities")
    List<@NotNull OnlineActivity> onlineActivities,

    @ArraySchema(
        schema = @Schema(implementation = FinancialChannel.class),
        arraySchema = @Schema(description = "질문4: 금융 채널 (다중선택, NONE은 배타 선택)", example = "[\"MOBILE_BANKING\",\"EASY_PAY\"]"))
    @NotEmpty(message = "financialChannels는 1개 이상 선택해야 합니다")
    @JsonAlias("financialChannels")
    @JsonProperty("financial_channels")
